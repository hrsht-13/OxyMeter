...
data=pd.read_csv("/content/body_measurements.csv")
...
if (rad=="Home"):
  ...
  with header:
    st.title("OxiMeter Web App")
    st.header("Hello and Welcome to the oximeter web app.")
    photo=Image.open("/content/home.jpg")
...
if (rad=="Important Ranges"):
  ...
  with header:
    ...
    photo=Image.open("/content/body-mass-index.jpg")
    st.image(photo,use_column_width=True)

    st.header("2. Temperature")
    photo=Image.open("/content/temp.jpg")
    st.image(photo,use_column_width=True)

    st.header("3. Blood Pressure")
    photo=Image.open("/content/press.jpg")
    st.image(photo,use_column_width=True)

    st.header("4. Oxygen Saturation")
    photo=Image.open("/content/oxygen-saturation.jpg")
    st.image(photo,use_column_width=True)
...
if (rad=="Error Analysis"):
    ...
    photo=Image.open("/content/residual.jpg")
    st.image(photo,use_column_width=True)

    st.header("2. Prediction Error")
    photo=Image.open("/content/prediction_error.jpg")
    st.image(photo,use_column_width=True)

    st.header("3. Cooks Distance")
    photo=Image.open("/content/cook's distance.jpg")
    st.image(photo,use_column_width=True)

    st.header("4. Feature Selection")
    photo=Image.open("/content/feature_selection.jpg")
    st.image(photo,use_column_width=True)

    st.header("5. Learning Curve")
    photo=Image.open("/content/learning_curve.jpg")
    st.image(photo,use_column_width=True)

    st.header("6. Manifolds Learning")
    photo=Image.open("/content/manifolds.jpg")
    st.image(photo,use_column_width=True)

    st.header("7. Validation Curve")
    photo=Image.open("/content/validation.jpg")
    st.image(photo,use_column_width=True)

    st.header("8. Feature Importance")
    photo=Image.open("/content/fea_imp1.jpg")
    st.image(photo,use_column_width=True)

    st.header("9. Interpret Model")
    photo=Image.open("/content/interpret.jpg")
    st.image(photo,use_column_width=True)

    st.header("10. Model Prediction")
    st.subheader("With a train-test split of 70:30")
    photo=Image.open("/content/prediction.jpg")
    st.image(photo,use_column_width=True)
